...
public class TestExecution_DeleteCycle_ApplyNotification_2Browsers_878850 extends LaunchBrowser
{
	LaunchBrowser lb;
	LoginPage lp;
	ProjectPage pp;
	...
	boolean[] actual=new boolean[25];
	  ...
	  String fileName="TestExecution_DeleteCycle_ApplyNotification_2Browsers_878850";
...
//				rl.reLogin();
	  ...
	  public void in_first_Browser_Creates_phase_with_name_and_Description() throws Throwable 
	  {
		  try
	      {
	    	  ...
	    	  String Name=UNIQUE+Excel_Lib.getData(INPUT_PATH,"TP_Phases",32,0);
	    	  String Desc=UNIQUE+Excel_Lib.getData(INPUT_PATH,"TP_Phases",32,1);
...
//				rl.reLogin();
	  ...
	  public void navigates_to_Created_PhAse_and_Adds_the_Testcase_with_Name_and_Description() throws Throwable 
	  {
		  try
	      {
	    	  ...
	    	  navigation[0]=UNIQUE+Excel_Lib.getData(INPUT_PATH,"TP_Phases",32,0);
...
//				rl.reLogin();
	  ...
	  public void launch_the_TestplaNning_and_Creates_the_TestExicution_Cycle() throws Throwable 
	  {
		  try
	      {
	    	  ...
	    	  bp.waitForElement();
	    	  String cycle=UNIQUE+Excel_Lib.getData(INPUT_PATH,"Exe Cycle",58,0);
	    	  String Build=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH,"Exe Cycle",58,1));
	    	  String environ=Excel_Lib.getData(INPUT_PATH,"Exe Cycle",58,2);
	    	  String hide=Excel_Lib.getData(INPUT_PATH,"Exe Cycle",58,100);
	    	  actual[15]=tp.CreateCycle(cycle, Build, environ, hide);
	    	  String syear=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH,"Exe Cycle",58,3));
	    	  String smonth=Excel_Lib.getData(INPUT_PATH,"Exe Cycle",58,4);
	    	  String sday=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH,"Exe Cycle",58,5));
	    	  String eyear=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH,"Exe Cycle",58,6));
	    	  String emonth=Excel_Lib.getData(INPUT_PATH,"Exe Cycle",58,7);
	    	  String eday=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH,"Exe Cycle",58,8));
...
//				rl.reLogin();
	  ...
	  public void adds_Phase_to_the_CyclE_and_Assign_to_Manager() throws Throwable 
	  {
		  try
	      {
	    	  ...
	    	  cycleName[0]=UNIQUE+Excel_Lib.getData(INPUT_PATH,"Exe Cycle",58,0);
	    	  actual[17]=tp.navigateToCycle(cycleName);
	    	  bp.waitForElement();
	    	  String Phase=UNIQUE+Excel_Lib.getData(INPUT_PATH,"TP_Phases",32,0);
	    	  actual[18]= tp.addPhaseToCycle(Phase);
	    	  bp.waitForElement();
	    	  String Bulk_type=Excel_Lib.getData(INPUT_PATH,"Exe Cycle",58,9);
	    	  actual[19]=tp.bulkAssignment(Bulk_type);
	    	  bp.waitForElement();
	    	  String phaseName=UNIQUE+Excel_Lib.getData(INPUT_PATH,"TP_Phases",32,0);
...
//				rl.reLogin();
	  ...
	  public void in_first_Browser_Delete_Cycle() throws Throwable 
	  {
		  try
	      {
			  BasePage bp=new BasePage();
	    	  ...
	    	  bp.waitForElement();
	    	  String cycleName=UNIQUE+Excel_Lib.getData(INPUT_PATH,"Exe Cycle",58,0);
	    	  actual[22]=tp.doubleClickOnCycle(cycleName);
	    	  actual[23]=tp.deleteNode();
	      }
	      catch(Exception e)
			{
	    	  lb.getScreenShot(fileName);
...
//				rl.reLogin();
	  ...
	  public void in_Second_Browser_Apply_Notifications() throws Throwable 
	  {
		  try
	      {
			  BasePage bp=new BasePage();
	    	  ...
	    	  log.info("Pass - Notifications are applied after deleting the Phase, in another Browser");
	    	  bp.waitForElement();
	    	  ProjectPage pp1=new ProjectPage(driver2);
