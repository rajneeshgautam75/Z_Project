...
public class External_JIRA_78 extends LaunchBrowser {
	LaunchBrowser lb=new LaunchBrowser();
	String fileName="External_JIRA_78";
	...
	String release=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_REQ, "Release_78");
	String importedTree=Excel_Lib.getData(INPUT_PATH_5, "Req", 1, 6);
	String requirementNode=UNIQUE+Excel_Lib.getData(INPUT_PATH_5, "Req", 14, 4);
	...
	public void user_is_in_Requirementpage() throws Throwable {
		try
		{
			...
			rrp.clickOnManageRelease();
			bp.waitForElement();
			
			boolean res=rrp.checkAvailableRelease(release);
			if(res==false)
			{
			String Desc=Excel_Lib.getData(INPUT_PATH_5, "Req", 100,100);
			String Hide=Excel_Lib.getData(INPUT_PATH_5, "Req", 100,100);
			
			  
			String MapexternalDefect=Excel_Lib.getData(INPUT_PATH_5, "Req", 100,100);
			rrp.addNewRelease(release, Desc, Hide, MapexternalDefect);
			String syear=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH_5, "Cycle Date", 3, 0));
			String smonth=Excel_Lib.getData(INPUT_PATH_5, "Cycle Date", 3, 1);
			String sday=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH_5, "Cycle Date", 12, 5));
			String eyear=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH_5, "Cycle Date", 3, 3));
			String emonth=Excel_Lib.getData(INPUT_PATH_5, "Cycle Date", 3, 4);
			String eday=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH_5, "Cycle Date", 1, 5));
			rrp.enterStartDatAndEndDateForRelease(syear, smonth, sday, eyear, emonth, eday);
			rrp.clickOnAdd();
			   }
			bp.waitForElement();
			pp.selectProject(project);
			pp.selectRelease(release);
			rrp.clickOnTestRep();
			bp.waitForElement();
			...
			node[0]=UNIQUE+Excel_Lib.getData(INPUT_PATH_5,"Req", 2, 8);
			actual[1]=tr.navigateToNode(release, node);
			tr.addTestCase();
			tr.addTestCase();
			bp.waitForElement();
...
//			String password=Excel_Lib.getData(INPUT_PATH_5, "LogIn",1 ,1 );
	...
	public void delete_the_release() throws Throwable {
		try
		{
			int tabNo=Excel_Lib.getNumberData(INPUT_PATH_5, "Req", 1, 9);
			bp.switchWindowTab(tabNo);
			...
			actual[35]=pp.deleteRelease(project, release);
			bp.waitForElement();
	...
	public void remote_Link_should_be_Deleted_from_JIRA() throws Throwable {
		try
		{
			...
			for(int i=0;i<=4;i++)
			{
		     ...
		     bp.waitForElement();
			 actual[36]=ej.validateLinkInExternalJira(testCaseNo);
			 bp.waitForElement();
			 actual[37]=ej.logOutFromExternal_JIRA();
			 bp.waitForElement();
			 driver.close();
			 bp.waitForElement();
