...
//	public void create_One_normal_project() throws Throwable {
		try
		{
		  ...
		  String project_10=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_DASH, "Normal_Project_9");
		  boolean result9=pp.validateProject(project_10);
		 	
		  if(result9==false)
		   {
		  String typeName=Excel_Lib.getData(INPUT_PATH, "Project_Data", 1, 9);
		  actual[2]=pp.createProject(project_10, description,typeName, lead, mapDefectProject, startDate, endDate, shareCheckbox);
		  bp.waitForElement();
		   }
		  
		  String project_11=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_DASH, "Normal_Project_9");
		  boolean result10=pp.validateProject(project_11);
		 	
		  if(result10==false)
		   {
		  String typeName=Excel_Lib.getData(INPUT_PATH, "Project_Data", 1, 9);
		  actual[2]=pp.createProject(project_11, description,typeName, lead, mapDefectProject, startDate, endDate, shareCheckbox);
		  bp.waitForElement();
		   }
		}
		catch(Exception e)
