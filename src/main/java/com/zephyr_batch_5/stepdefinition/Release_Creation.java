...
public class Release_Creation extends LaunchBrowser{
	...
	public void user_select_the_project(int arg1) throws Throwable {
	   try
	   {
		   ...
		   {
			   String projectName=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_DT,"Normal_Project_2");
			   pp=new ProjectPage(driver);
			   pp.selectProject(projectName);
		   ...
		   {
			   String projectName=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_DT,"Restricted_Project_1");
			   pp=new ProjectPage(driver);
			   pp.selectProject(projectName); 
		   }
		   else if(arg1==4)
		   {
			   String projectName=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_DT,"Restricted_Project_2");
			   pp=new ProjectPage(driver);
			   pp.selectProject(projectName); 
		   }
		   else if(arg1==5)
		   {
			   String projectName=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_DT,"Isolated_Project_1");
			   pp=new ProjectPage(driver);
			   pp.selectProject(projectName); 
		   }
	   }
	...
	public void user_create_a_release_for_all_the_projects() throws Throwable {
	    try
	    {
			...
			bp.waitForElement();
			String releaseName=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_DT,"Release_2");
