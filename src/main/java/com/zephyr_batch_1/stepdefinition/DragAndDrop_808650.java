...
public class DragAndDrop_808650 extends LaunchBrowser
{
	...
	String fileName="DragAndDrop_808650";
	...
	public void navigate_to_restricted_project_and_imported() throws Throwable 
	{
		try
		{
		bp.waitForElement();
	...
	@When("^drag and drop to P/S/SS in current release$")
	public void drag_and_drop_to_P_S_SS_in_current_release() throws Throwable 
	{
		try
		{
	   ...
	   actual[8]= cp.dragAndDropImportedNodes(impnode1, phase1);
	    ...
	    if(actual[8]=true)
	    {
	    	log.info("imported node is sucessfully drag and droped into phase");
	    }
	  //drag and drop to system
	   ...
	   if(actual[9]=true)
	    {
	    	log.info("imported node is sucessfully drag and droped into System");
	    }
	    bp.waitForElement();
	    bp.waitForElement();
		  //drag and drop to system
		   ...
		   actual[10]=cp.dragAndDropImportedNodes(impnode1, subsystem);
		    ...
		    if(actual[10]=true)
		    {
		    	log.info("imported node is sucessfully drag and droped into Subsystem");
		    }
		}catch (Exception e) 
		{
			lb.getScreenShot(fileName);
