...
public class Map_808663 extends LaunchBrowser {
	...
	public void user_allocates_Imported_folder_to_Release_node() throws Throwable {
		...
		try
		{
		   ...
		   // tr.doubleClickOnRelease(releaseName2);
			...
			req=new RequirementsPage(driver);
			bp.waitForElement();
			actual[18]=tr.dragAndDropNode(srcNode, destNode);
		}
		catch(Exception e)
		{
			lb=new LaunchBrowser();
	...
	public void user_clones_the_Jira_requirement() throws Throwable {
		...
		//ip.validateImport(fileName, releaseName);
			...
			//bp.waitForElement();
			//req=new RequirementsPage(driver);
			actual[21]=req.selectMultipleRequirementFromGrids(testCaseNo);
		 	bp.waitForElement();
			...
			bp.waitForElement();
			req.selectMultipleRequirementFromGrids(testCaseNo1);
			bp.waitForElement();
		}
	    catch(Exception e)
		{
	...
	public void user_again_maps_the_cloned_Jira_requirement_to_same_testcase() throws Throwable {
	 try
	 {
		...
		testCaseNo[1]=Integer.toString(Excel_Lib.getNumberData(INPUT_PATH_3, "TestCases",2 ,4 ));
		bp.waitForElement();
		actual[27]=req.selectMultipleRequirementFromGrids(testCaseNo);
		bp.waitForElement();
